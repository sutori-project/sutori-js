...
class SutoriDocument{constructor(){this.Actors=new Array();this.Moments=new Array();}
static async LoadXml(uri){const result=new SutoriDocument();await result.AddDataFromXmlUri(uri);return result;}
async AddDataFromXmlUri(uri){const response=await fetch(uri);const raw_xml=await response.text();console.log("loading moments from "+uri);this.AddDataFromXml(raw_xml);}
AddDataFromXml(raw_xml){const xml_parser=new DOMParser();const xml=xml_parser.parseFromString(raw_xml,"text/xml");const self=this;xml.querySelectorAll('actors actor').forEach((actor_e)=>{self.Actors.push(SutoriActor.Parse(actor_e));});xml.querySelectorAll('moments moment').forEach((moment_e)=>{const moment=new SutoriMoment();if(moment_e.hasAttribute('id')){moment.ID=moment_e.attributes['id'].textContent;}
if(moment_e.hasAttribute('goto')){moment.Goto=moment_e.attributes['goto'].textContent;}
self.AddMomentAttributes(moment,moment_e,['id','goto']);moment_e.querySelectorAll(':scope > *').forEach(async(element_e)=>{switch(element_e.tagName){case'text':moment.Elements.push(SutoriElementText.Parse(element_e));break;case'option':moment.Elements.push(SutoriElementOption.Parse(element_e));break;case'image':moment.Elements.push(SutoriElementImage.Parse(element_e));break;case'audio':moment.Elements.push(SutoriElementAudio.Parse(element_e));break;case'video':moment.Elements.push(SutoriElementVideo.Parse(element_e));break;case'load':const loader=SutoriElementLoad.Parse(element_e);if(loader.LoadMode==SutoriLoadMode.Immediate){await self.AddDataFromXmlUri(loader.Path);loader.Loaded=true;}
...
throw new Error("Document does not have any beads!");this.Cursor=moment;const loaderElements=moment.GetLoaderElements(SutoriLoadMode.OnEncounter);if(loaderElements&&loaderElements.length>0){for(let i=0;i<loaderElements.length;i++){await self.Document.AddDataFromXmlUri(loaderElements[i].Path);loaderElements[i].Loaded=true;}}
...
class SutoriMoment{constructor(){this.Attributes=new Object;this.Elements=new Array();this.Goto='';this.ID='';}
...
static Parse(element){const result=new SutoriElementImage();result.Src=element.textContent;result.ParseExtraAttributes(element,['actor','purpose','lang']);if(element.hasAttribute('actor')){result.Actor=element.attributes['actor'].textContent;}
if(element.hasAttribute('purpose')){result.Purpose=element.attributes['purpose'].textContent;}
if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
return result;}
GetAssociatedActor(document){if(this.Actor==null)
return null;return document.Actors.find(t=>t.ID==this.Actor);}}
...
static Parse(element){const result=new SutoriElementLoad();result.Path=element.textContent;result.ParseExtraAttributes(element,['mode']);if(element.hasAttribute('mode')){const mode=element.attributes['mode'].textContent;result.LoadMode=SutoriTools.ParseLoadMode(mode);}
return result;}}
...
static Parse(element){const result=new SutoriElementText();result.Text=element.textContent;result.ParseExtraAttributes(element,['actor','lang']);if(element.hasAttribute('actor')){result.Actor=element.attributes['actor'].textContent;}
if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
return result;}
GetAssociatedActor(document){if(this.Actor==null)
return null;return document.Actors.find(t=>t.ID==this.Actor);}}
