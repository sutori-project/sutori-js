...
static async LoadXml(uri){const response=await fetch(uri);const xml_raw=await response.text();const xml_parser=new DOMParser();const xml=xml_parser.parseFromString(xml_raw,"text/xml");const result=new VnsDocument();xml.querySelectorAll('moments moment').forEach((moment_e)=>{const moment=new VnsMoment();if(moment_e.hasAttribute('id')){moment.ID=moment_e.attributes['id'].textContent;}
if(moment_e.hasAttribute('goto')){moment.Goto=moment_e.attributes['goto'].textContent;}
moment_e.querySelectorAll('elements > *').forEach((element_e)=>{switch(element_e.tagName){case'text':moment.Elements.push(VnsElementText.Parse(element_e));break;case'option':moment.Elements.push(VnsElementOption.Parse(element_e));break;case'image':moment.Elements.push(VnsElementImage.Parse(element_e));break;case'audio':moment.Elements.push(VnsElementAudio.Parse(element_e));break;case'video':moment.Elements.push(VnsElementVideo.Parse(element_e));}});result.Moments.push(moment);});return result;}
...
static ParseSolver(cultureName){var _a;const stringKey=(_a=Object.entries(VnsSolver).find(([key,val])=>val===cultureName))===null||_a===void 0?void 0:_a[0];return VnsSolver[stringKey];}}
