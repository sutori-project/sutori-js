...
GetElements(culture,type){const elements=typeof culture=='undefined'?this.Moment.Elements.filter(e=>e instanceof type):this.Moment.Elements.filter(e=>e instanceof type&&(e.ContentCulture==culture||e.ContentCulture==SutoriCulture.All));return elements;}
GetText(culture){return this.GetElements(culture,SutoriElementText);}
...
class SutoriDocument{constructor(){this.Moments=new Array();}
static async LoadXml(uri){const result=new SutoriDocument();await result.AddMomentsFromXmlUri(uri);return result;}
async AddMomentsFromXmlUri(uri){const response=await fetch(uri);const raw_xml=await response.text();console.log("loading moments from "+uri);this.AddMomentsFromXml(raw_xml);}
AddMomentsFromXml(raw_xml){const xml_parser=new DOMParser();const xml=xml_parser.parseFromString(raw_xml,"text/xml");const self=this;xml.querySelectorAll('moments moment').forEach((moment_e)=>{const moment=new SutoriMoment();if(moment_e.hasAttribute('id')){moment.ID=moment_e.attributes['id'].textContent;}
if(moment_e.hasAttribute('goto')){moment.Goto=moment_e.attributes['goto'].textContent;}
self.AddMomentAttributes(moment,moment_e,['id','goto']);moment_e.querySelectorAll('elements > *').forEach(async(element_e)=>{switch(element_e.tagName){case'text':moment.Elements.push(SutoriElementText.Parse(element_e));break;case'option':moment.Elements.push(SutoriElementOption.Parse(element_e));break;case'image':moment.Elements.push(SutoriElementImage.Parse(element_e));break;case'audio':moment.Elements.push(SutoriElementAudio.Parse(element_e));break;case'video':moment.Elements.push(SutoriElementVideo.Parse(element_e));break;case'load':const loader=SutoriElementLoad.Parse(element_e);if(loader.LoadMode==SutoriLoadMode.Immediate){await self.AddMomentsFromXmlUri(loader.Path);loader.Loaded=true;}
moment.Elements.push(loader);break;}});self.Moments.push(moment);});}
...
static Parse(element){const result=new SutoriElementAudio();result.Src=element.textContent;result.ParseExtraAttributes(element,['lang']);if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
return result;}}
class SutoriElementImage extends SutoriElement{constructor(){super();this.ContentCulture=SutoriCulture.None;}
static Parse(element){const result=new SutoriElementImage();result.Src=element.textContent;result.ParseExtraAttributes(element,['lang']);if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
return result;}}
class SutoriElementLoad extends SutoriElement{constructor(){super();this.LoadMode=SutoriLoadMode.Immediate;this.ContentCulture=SutoriCulture.None;this.Loaded=false;}
static Parse(element){const result=new SutoriElementLoad();result.Path=element.textContent;if(element.hasAttribute('mode')){const mode=element.attributes['mode'].textContent;result.LoadMode=SutoriTools.ParseLoadMode(mode);}
return result;}}
...
static Parse(element){const result=new SutoriElementVideo();result.Src=element.textContent;result.ParseExtraAttributes(element,['lang']);if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
return result;}}
...
throw new Error("Document does not have any beads!");this.Cursor=moment;const loaderElements=moment.GetLoaderElements(SutoriLoadMode.OnEncounter);if(loaderElements&&loaderElements.length>0){for(let i=0;i<loaderElements.length;i++){await self.Document.AddMomentsFromXmlUri(loaderElements[i].Path);loaderElements[i].Loaded=true;}}
...
class SutoriMoment{constructor(){this.Elements=new Array();this.Attributes=new Object;this.ID='';this.Goto='';}
...
AddImage(culture,src){const element=new SutoriElementImage();element.ContentCulture=culture;element.Src=src;this.Elements.push(element);return element;}
...
GetLoaderElements(mode){const elements=this.Elements.filter(e=>e instanceof SutoriElementLoad);const casted=elements;return casted.filter(e=>e.LoadMode==mode);}}
class SutoriTools{static ParseCulture(cultureName){var _a;const stringKey=(_a=Object.entries(SutoriCulture).find(([key,val])=>val===cultureName))===null||_a===void 0?void 0:_a[0];return SutoriCulture[stringKey];}
