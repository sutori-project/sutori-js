...
async AddDataFromXmlUri(uri){const response=await fetch(uri);const raw_xml=await response.text();console.log("loading moments from "+uri);await this.AddDataFromXml(raw_xml);}
async AddDataFromXml(raw_xml){const xml_parser=new DOMParser();const xml=xml_parser.parseFromString(raw_xml,"text/xml");const self=this;const includes=xml.querySelectorAll('include');for(let i=0;i<includes.length;i++){const include=includes[i];if(include.hasAttribute('after')===false){await this.AddDataFromXmlUri(include.textContent);}}
xml.querySelectorAll('actors actor').forEach((actor_e)=>{self.Actors.push(SutoriActor.Parse(actor_e));});xml.querySelectorAll('moments moment').forEach((moment_e)=>{const moment=new SutoriMoment();if(moment_e.hasAttribute('id')){moment.ID=moment_e.attributes['id'].textContent;}
...
if(element.hasAttribute('lang')){const lang=element.attributes['lang'].textContent;result.ContentCulture=SutoriTools.ParseCulture(lang);}
if(element.hasAttribute('preload')){const preload=SutoriTools.ParseBool(element.attributes['preload'].textContent);result.Preload=preload;if(preload===true){const img=new Image();img.src==result.Src;}}
else{result.Preload=false;}
return result;}
GetAssociatedActor(document){if(this.Actor==null)
return null;return document.Actors.find(t=>t.ID==this.Actor);}}
