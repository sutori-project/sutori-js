...
xml.querySelectorAll('resources > *').forEach((resource_e)=>{if(resource_e.tagName=='image'){self.Resources.push(SutoriResourceImage.Parse(resource_e));}});xml.querySelectorAll('actors actor').forEach((actor_e)=>{self.Actors.push(SutoriActor.Parse(actor_e));});xml.querySelectorAll('moments moment').forEach((moment_e)=>{const moment=new SutoriMoment();const moment_ex=new HTMLElementEx(moment_e);self.AddMomentAttributes(moment,moment_e,['id','goto','actor','clear']);moment.ID=moment_ex.readAttribute('id');moment.Goto=moment_ex.readAttribute('goto');moment.Actor=moment_ex.readAttribute('actor');moment.Clear=moment_ex.readAttributeBool('clear',false);moment_e.querySelectorAll(':scope > *').forEach(async(element_e)=>{switch(element_e.tagName){case'text':moment.Elements.push(SutoriElementText.Parse(element_e));break;case'option':moment.Elements.push(SutoriElementOption.Parse(element_e));break;case'image':moment.Elements.push(SutoriElementImage.Parse(element_e));break;case'audio':moment.Elements.push(SutoriElementAudio.Parse(element_e));break;case'video':moment.Elements.push(SutoriElementVideo.Parse(element_e));break;case'load':moment.Elements.push(SutoriElementLoad.Parse(element_e));break;case'set':moment.Elements.push(SutoriElementSet.Parse(element_e));break;case'trigger':moment.Elements.push(SutoriElementTrigger.Parse(element_e));break;}});self.Moments.push(moment);});for(let i=0;i<self.Includes.length;i++){const include=self.Includes[i];if(include.After===true){await this.AddDataFromXmlUri(include.Path);}}}
...
GotoNextMoment(){const self=this;console.log('Goto next moment.');if(self.Cursor==null)
return false;const index=self.Document.Moments.indexOf(self.Cursor);if(index==-1)
return false;if(!SutoriTools.IsEmptyString(self.Cursor.Goto)){self.GotoMomentID(self.Cursor.Goto);return false;}
...
static Parse(element){const result=new SutoriElementImage();const element_ex=new HTMLElementEx(element);result.ParseExtraAttributes(element,['actor','for','resource','lang']);result.Actor=element_ex.readAttribute('actor');result.For=element_ex.readAttribute('for');result.ResourceID=element_ex.readAttribute('resource');result.ContentCulture=element_ex.readAttributeCulture('lang');return result;}
GetAssociatedActor(document){if(this.Actor==null)
return null;return document.Actors.find(t=>t.ID==this.Actor);}}
...
static Parse(element){const result=new SutoriElementVideo();const element_ex=new HTMLElementEx(element);result.ParseExtraAttributes(element,['actor','for','resource','lang']);result.Actor=element_ex.readAttribute('actor');result.For=element_ex.readAttribute('for');result.ResourceID=element_ex.readAttribute('resource');result.ContentCulture=element_ex.readAttributeCulture('lang');return result;}
GetAssociatedActor(document){if(this.Actor==null)
return null;return document.Actors.find(t=>t.ID==this.Actor);}}
